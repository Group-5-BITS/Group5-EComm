<template>
  ...
  <!-- Cart -->
  <div class="head-cart" v-if="showCart">
    <h3 class="head-cart__heading">Cart</h3>
    <div class="head-cart__txt" v-if="store.cart.length === 0">
      Your cart is empty.
    </div>
    <div class="head-cart__item" v-else>
      <div
        class="head-cart__item-wrapper"
        v-for="(item, index) of store.cart"
        :key="item.name"
      >
        <img
          :src="item.images[0]"
        ...
        <div class="head-cart__des">
          <p class="head-cart__des-txt">{{ item.name }}, {{ index }}s</p>
          <div class="head-cart__price">
            <span class="head-cart__price-single"
              >${{ item.price.value }}*{{ item.count }}</span
            >
            <span class="head-cart__price-total"
              >${{ item.price.value * item.count }}</span
            >
          </div>
        </div>
        <button class="head-cart__item-btn" @click="removeFromCart(index)">
          <img
...
<nav-bar/>
  ...
  <header class="head">
    <div class="head-lft">
      ...
      />
      <nav class="head-nav">
        <router-link to="/" class="head-nav__item">Home</router-link>
      </nav>
      <!--      <ul class="head-nav">-->
    ...
    <div class="head-rgt">
      <button class="head-rgt__btn" @click="toggleCart">
        <img
